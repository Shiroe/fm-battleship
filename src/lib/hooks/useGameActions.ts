...
export function useGameActions() {
  ...
  const checkWinCondition = useCombinedStore((state) => state.checkWinCondition);
  const showSunkShipModal = useCombinedStore((state) => state.showSunkShipModal);
  ...
  }, [fireAt, incrementRound, recordShipHit, checkWinCondition, showSunkShipModal]);
